...
// add document with types
/**
 * @description Controller xử lý logic login
...
const logout = async (req, res, next) => {
  try {
    ...
    res.status(StatusCodes.OK).json({ message: 'Logout successfully', loggedOut: true })
  } catch (error) { next(error) }
}
...
const refreshToken = async (req, res, next) => {
  try {
    ...
    res.status(StatusCodes.OK).json(result)
  } catch (error) { next(new ApiError(StatusCodes.UNAUTHORIZED, 'Unauthorized')) }
}
